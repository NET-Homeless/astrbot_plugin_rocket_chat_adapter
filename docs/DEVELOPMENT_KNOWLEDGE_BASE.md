...
- 某条加密消息解不开，只跳过那一条
- 如果 `rooms.info` 解析不出房间类型（REST 失败且无缓存，常见于主动 `send_by_session` 到一个从未收到过消息的房间），发送侧不能把未知房间默认成未加密公开频道（旧兜底是 `{t:"c", encrypted:False}`），而要 fail-closed 拒发——否则该房间一旦实际是加密房间，就会误走明文。这与密钥缺失无关：密钥问题在入站解密侧处理，这里防的是“类型未知却按明文默认”
