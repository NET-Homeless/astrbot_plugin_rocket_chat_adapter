...
开发与排障知识沉淀见：[docs/DEVELOPMENT_KNOWLEDGE_BASE.md](docs/DEVELOPMENT_KNOWLEDGE_BASE.md)
...
| 其他富媒体语义 | 转发等 | Markdown 文本降级展示 | ⚠️ 降级处理 | `Nodes/Node/Forward` 会展示为可读文本；E2EE 房间按加密文本发送；不保证语义等价 |
...
- **严格支持（✅）**
  ...
  - 运行：房间订阅变更（动态订阅）
- **降级支持（⚠️）**
  - 转发等其他富媒体语义：以 Rocket.Chat Markdown 文本展示，不保证语义等价
- **不支持（❌）**
  - 系统/审计事件
...
| 其他富媒体语义仍为降级支持 | 功能边界 | 当前严格支持覆盖文本、图片、普通文件、语音、视频和输入中状态；转发等其余富媒体语义会尽量转为可读 Markdown 文本。 |
| 加密远程媒体依赖源地址可下载 | 行为约束 | 加密房间中的远程图片/语音/视频会先下载再上传；如果源地址不可下载，会降级为加密文本链接。 |
| 房间信息不可确认时拒发 | 行为约束 | 主动 `send_by_session` 到一个 bot 从未收到过消息的房间时，需先用 `rooms.info` 解析房间类型；若该查询失败且本地无缓存，适配器不会把未知房间默认当成公开明文频道发送，而是拒发该条消息/媒体（fail-closed）。回复已收到的消息不受影响。 |
